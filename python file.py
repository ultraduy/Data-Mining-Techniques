...
data_genderandstress = data.iloc[:,[6,12]]
data_genderandstress.rename(columns={'What is your gender?':'Gender',"You can get 100 euros if you win a local DM competition, or we don’t hold any competitions and I give everyone some money (not the same amount!). How much do you think you would deserve then? ":'Deserve'}, inplace=True)
data_genderandstress= data_genderandstress[check_integer(data_genderandstress['Deserve'])]
data_genderandstress['Deserve'] = pd.to_numeric(data_genderandstress['Deserve'])
df_filtered = data_genderandstress[data_genderandstress['Deserve'] <=100  ]
df_filtered = df_filtered[df_filtered['Deserve'] >=0  ]
...
mX = pre_data[["What is your stress level (0-100)?", "You can get 100 euros if you win a local DM competition, or we don’t hold any competitions and I give everyone some money (not the same amount!). How much do you think you would deserve then? "]]
mX = mX.astype('int')
print(np.corrcoef(mX.iloc[:,0],mX.iloc[:,1]))
#Naive Bayes
#split dataset into train and test data
X_train, X_test, y_train, y_test = train_test_split(mX.drop(55), vY.drop(55), test_size=0.2, random_state=1, stratify=vY.drop(55)) #gender at 55 only comes across 1 time which causes errors for KNN
clf = MultinomialNB()
clf.fit(X_train, y_train)
vY_nb = clf.predict(X_test)
print(succes_rate(y_test.to_numpy(), vY_nb))
